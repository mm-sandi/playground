...
Check today's day of the week (Asia/Tokyo timezone):
- **If Monday**: use `after:friday` for Slack and `now("-3d")` for Confluence — this covers the full weekend
- **All other weekdays**: use `after:yesterday` for Slack and `now("-1d")` for Confluence
...
- Query: `after:friday` (on Mondays) or `after:yesterday` (Tue–Fri)
...
## Step 3: Fetch JIRA MINT Mentions (past 7 days)

Use the `searchJiraIssuesUsingJql` tool with this JQL:
```
project = MINT AND (text ~ "kurniawan" OR text ~ "Sandi" OR text ~ "712020:78925e0b-c576-41e1-9e76-5b3559302fb9") AND updated >= -7d ORDER BY updated DESC
```
...
## Step 4: Fetch Confluence Mentions

Use the `searchConfluenceUsingCql` tool with this CQL:
- Monday: `type = comment AND (text ~ "kurniawan" OR text ~ "Sandi") AND created >= now("-3d")`
- Tue–Fri: `type = comment AND (text ~ "kurniawan" OR text ~ "Sandi") AND created >= now("-1d")`
