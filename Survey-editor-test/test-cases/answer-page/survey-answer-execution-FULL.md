...
**Status:** DRAFT              
**Count:** ~736 active cases + ~19 out-of-scope stubs (SC-014/015/017/018 moved to OOS) — EX (24), SC (12 active + 4 OOS), FC (25), VA (24), C1/C5 (9), EX-LOGIC (95), V-PAGEBREAK-01–31 (31), V-FA-001–008 (8), V-FAL-001–003 (3), V-RAT-CS-001–008 (8), CM-001–012 (12), C-2LOGIC-073–123 (51), C-QTSEL-001–152 (152), C-QTRAND-001–078 (78), C-MATRIX-COUNT-31–63 (33), C-SCREENING-001–024 (24), C-DISABLED-001–026 (26), C-TXTBOX-001–029 (29), V-REQ-Q-01–13 (13), V-REQ-SQ-01–20 (20), V-SPECN-01–38 (38)      
...
| EX-008 | Constant Sum (RAT) Slider Sub-control, Value Dragged, Submit | RAT (Constant Sum / percent-total mode) question with slider input; 4 items, total=100 | Drag slider for item-1 to ~70%; observe snap behavior | Slider snaps to nearest 10% increment (70%); value recorded; remaining items adjusted automatically; Survey advances on valid total | Note: "Slider" is not a standalone QT — it is the slider sub-control within RAT Constant Sum Percent mode. Slider snaps in 10% increments per spec. |
...
| EX-017 | Ranking Question (RNK), All Items Ranked, Submit | Ranking question with 4 items | Assign ranks to order: B=1, D=2, A=3, C=4 → click Submit | Final order B > D > A > C is shown; Order is recorded correctly; Survey advances | Note: "Drag items" step changed to "Assign ranks" — drag-and-drop may not be the actual interaction; verify against FE implementation (could be numbered radio buttons or dropdown per item). |
| EX-018 | Radio (SAR) with Image Choices, Image Selected, Submit | SAR question with image attachments on each choice (3 choices, each with an image) | Click on image/choice 2 → click Submit | Choice 2 visually highlighted/selected; choice 2 recorded; Survey advances | Note: "Image Choice Question" is not a standalone QT. This tests image rendering on choices within SAR (or MAC). Renamed to clarify the underlying QT is SAR. |
| EX-019 | Matrix Checkbox (MTM), Multiple Cells Selected, Submit | MTM (Matrix Checkbox) 3 SQs × 3 choices | Check (SQ1, col2), (SQ2, col1), (SQ3, col3) → click Submit | All three checked cells are visually confirmed; All three are recorded; Survey advances | Note: Renamed from "Checkbox Grid" (non-system term) to "Matrix Checkbox (MTM)" per system QT naming. |
...
| SC-014 | ~~Screen-out Rate Tracking~~ *(Out of Scope — reporting/analytics metric, not FE executor behavior)* | — | — | — | OOS |
| SC-015 | ~~Soft Launch Limit, Respondent Count at Limit~~ *(Out of Scope — quota/entry-count cap is backend gatekeeping, not FE executor behavior)* | — | — | — | OOS |
...
| SC-017 | ~~Duplicate Respondent, Same Panel ID, Attempt Re-entry~~ *(Out of Scope — duplicate detection by panel ID is Monitor System / backend concern, not FE executor)* | — | — | — | OOS |
| SC-018 | ~~Expired Survey Link, Access Attempt~~ *(Out of Scope — expiry checking is pre-entry backend validation; FE executor not invoked on expired link)* | — | — | — | OOS |
...
| V-PAGEBREAK-28 | Four-Page Survey, Back from Page 3 After Skip Returns to Last Manually-Visited Page (Page 1) | Same as V-PAGEBREAK-27; respondent is on page 3 after page 2 was auto-skipped by logic | Click "Back" | Per spec: back navigation returns to the last page the respondent manually interacted with — which is page 1 (page 2 was skipped by logic, not manually visited); page 2 is NOT shown during back navigation; No crash or loop | |
...
| C-QTSEL-152 | MAC | choiceSelect+specN+textbox | MAC specN=exact:2 narrowed to exactly 2 but one remaining choice has a textbox — auto-select NOT triggered (Exception 1+2 combined) | MAC q1 required specN=exact:2 5 choices; choiceSelect hides 3 → 2 remain; one of the 2 has a required textbox attachment | displayed = specN count; BUT one remaining choice has a textbox; per spec Exception 1: auto-select does NOT fire when sole/remaining required choice has a textbox; question rendered; respondent must interact | |
...
| C-MATRIX-COUNT-40 | matrixInclusion-runtime | MTM | Inclusion + prohibition on same SQ pair — theoretical behavior | MTM q1; inclusion SQ1→SQ2 + prohibition SQ1↔SQ2; SQ1 selects choice-1 | Per combined-behavior spec Case A: this configuration (MI source = Prohibition source = same SQ pair) is "Not configurable" in the editor — the editor blocks this combination. This case is therefore unreachable in a properly-configured survey. Retained for theoretical reference only. If reached via API or legacy data: inclusion reveals choice-1 in SQ2; prohibition hides it; net: choice-1 not selectable in SQ2. | Note: Editor prevents this config per combined spec Case A (Not configurable). |
...
| C-MATRIX-COUNT-49 | prohibition-runtime | MTM | Inclusion + prohibition on same SQ pair — theoretical net result | MTM q1; inclusion SQ1→SQ2 + prohibition SQ1↔SQ2; SQ1 selects choice-1 | Per combined-behavior spec Case A: this is editor-blocked (Not configurable). Theoretical result if reached: inclusion reveals; prohibition hides; choice-1 not selectable in SQ2. | Note: Editor prevents this config per combined spec Case A. Unreachable in properly-configured survey. |
...
|V-RAT-CS-008|Optional RAT Out-of-10 mode — all items empty advances; any item non-zero enforces total=10|Create survey with Q1 (RAT, Out-of-10 mode, 5 items, required=OFF, totalValue=10). Publish.|(a) Open Answer. Leave all 5 items at 0. Click Next. |(a) All items 0 in optional mode → saved as unanswered; survey advances without error. |(b) Restart. Enter value=3 for item-1 only. Click Next. |(b) Total=3 ≠ 10 → ConstantSumError: `mint.survey_answer.error_summary_total_insufficient`; survey blocked.|NT||
...
|---|---|---|
| 2026-05-11 | Comprehensive third 3-pass review. **Pass 1 accuracy:** EX-008 reframed (Slider → RAT sub-control, 10%-snap behavior specified); EX-017 "Drag items" → "Assign ranks" (drag-and-drop unverified, note added); EX-018 "Image Choice Question" → "SAR with Image Choices" (not a standalone QT); EX-019 "Checkbox Grid" → "MTM (Matrix Checkbox)"; V-PAGEBREAK-28 ambiguity fixed ("or page 2" removed — spec says back always returns to last manually-visited page); C-MATRIX-COUNT-40/49 flagged as unreachable (editor-blocked per combined spec Case A); SC-014/015/017/018 moved to Out of Scope (reporting, quota, duplicate-check, expiry are backend concerns). **Pass 2:** +3 new cases: C-QTSEL-152 (MAC specN=exact:2, textbox exception), V-RAT-CS-008 (optional RAT Out-of-10). **Pass 3:** Count updated ~730 → ~736 active + ~19 OOS. | C-QTSEL-152, V-RAT-CS-008 |
