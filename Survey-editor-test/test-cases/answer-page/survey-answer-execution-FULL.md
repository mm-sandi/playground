...
**Status:** DRAFT              
**Count:** ~715 active cases + ~15 out-of-scope stubs — EX (24), SC (16), FC (25), VA (24), C1/C5 (9), EX-LOGIC (95), V-PAGEBREAK-01–31 (31), V-FA-001–008 (8), V-RAT-CS-001–003 (3), CM-001–012 (12), C-2LOGIC-073–123 (51), C-QTSEL-001–149 (149), C-QTRAND-001–078 (78), C-MATRIX-COUNT-31–60 (30), C-SCREENING-001–024 (24), C-DISABLED-001–026 (26), C-TXTBOX-001–029 (29), V-REQ-Q-01–10 (10), V-REQ-SQ-01–20 (20), V-SPECN-01–38 (38)      
...
| C-MATRIX-COUNT-45 | prohibition-runtime | MTM | Prohibition + specN: fewer choices may prevent specN | MTM q1; prohibition SQ1↔SQ2; SQ2 specN=exact:3; SQ1 selects 3 → 2 remain in SQ2 | only 2 in SQ2; per spec: disabled → specN ignored; or submit blocked | |
| C-MATRIX-COUNT-46 | prohibition-runtime | MTS | MTS does not support prohibition: no runtime behavior | MTS q1; prohibition configured (if editor allows) | prohibition not applicable to MTS; no hiding observed | |
...
| C-MATRIX-COUNT-50 | prohibition-runtime | MTM | All choices prohibited: no selections possible | MTM q1; prohibition between every pair; SQ1 selects all | all choices hidden from all other SQs; submit blocked if required | |
...
## FA/FAL Input Type — Answer Runtime (V-FA-001 – V-FA-004)

Tests **FA (Text Box) and FAL input type configuration** variants at Answer runtime. Covers type=Text maxLength boundary, type=Date validation, and type=Number min/max.
...
|V-FA-002|FA type=Date — invalid input → error; valid date → advances|Create survey with Q1 (FA, type=Date, required=ON). Publish.|Open Answer. (a) Enter "abcdef" (non-date string). Click Next. (b) Enter a valid date (e.g. "2025-03-15" or via date picker). Click Next.|(a) Validation error displayed: invalid date format. Q1 not advanced. (b) Survey advances. Date recorded correctly.|NT|https://macromill.atlassian.net/browse/MINT-5870, https://macromill.atlassian.net/browse/MINT-7200|
|V-FA-003|FA type=Number — min=1, no max — below min → error; at min and above → valid|Create survey with Q1 (FA, type=Number, min=1, no max configured, required=ON). Publish.|Open Answer. (a) Enter 0. Click Next. (b) Enter 1. Click Next. (c) Enter 99999. Click Next.|(a) Real-time or submit-time error: "requires a value of 1 or higher". (b) Valid; advances. (c) Valid; advances (no upper limit enforced).|NT|https://macromill.atlassian.net/browse/MINT-5870, https://macromill.atlassian.net/browse/MINT-4482, https://macromill.atlassian.net/browse/MINT-7200|
...
|V-RAT-CS-002|Overflow auto-adjustment: entering value causes auto-reduce of other items|Create survey with Q1 (RAT, percent-total=100, 5 items). Publish.|Open Answer. Enter 60% for item-1, 60% for item-2 (sum now 120%).|System auto-adjusts a previously entered item downward so total does not exceed 100%; adjustment is non-blocking (no error modal); total displayed remains at or near 100%; warning indicator may appear.|NT||
|V-RAT-CS-003|Submit blocked when sum is below required total|Create survey with Q1 (RAT, percent-total=100, 5 items, required=ON). Publish.|Open Answer. Enter 40% for item-1, 30% for item-2, leave items 3–5 at 0%. Total = 70%. Click Next.|Validation error: total must equal 100%; survey does NOT advance. Entering values that sum to exactly 100% allows submission.|NT||
