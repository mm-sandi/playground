...
| FA-ED-013 | FA type=Email — saves and format-validation is set | Add FA. Set type=Email. | Click Save. | Saves. Input type=Email stored. In Answer, invalid email format triggers validation error (per spec: `email` type input restricted to valid email format). | NT | |
| FA-ED-014 | FA type=Code (numeric-only keyboard) — saves | Add FA. Set type=Code. | Click Save. | Saves. Input type=Code stored. In Answer, numeric-only keyboard triggered; non-numeric input rejected. Distinct from type=Number (Code has no min/max; only digits accepted). | NT | |
| FA-ED-015 | FA type=Alnum (half-width alphanumeric) — saves | Add FA. Set type=Alnum. | Click Save. | Saves. Input type=Alnum stored. In Answer, only half-width alphanumeric characters accepted; full-width and non-alphanumeric characters rejected. | NT | |
| FA-ED-016 | FA placeholder toggle ON/OFF — saves | Add FA. Enable placeholder toggle (ON); enter placeholder text. Then switch toggle to OFF. | Click Save after each state change. | (a) Placeholder ON + text entered: saves; placeholder text persisted and displayed in Answer input field. (b) Placeholder OFF: saves; no placeholder shown in Answer regardless of previously-entered placeholder text. | NT | |
| FA-ED-017 | FAL required mode=All — saves | Add FAL (5 rows). Set required mode = All. | Click Save. | Saves. Required mode=All persisted: all 5 rows must be filled by respondent in Answer. | NT | |
| FA-ED-018 | FAL required mode=Custom (mark specific rows) — saves | Add FAL (5 rows). Set required mode = Custom; mark rows 1, 3, 5 as required. | Click Save. | Saves. Required mode=Custom + marked rows persisted. In Answer, only rows 1, 3, 5 are required. | NT | |
| FA-ED-019 | FAL required mode=Custom+Minimum (marked rows + N minimum) — saves | Add FAL (5 rows). Set required mode = Custom+Minimum; mark rows 1, 2, 3; set minimum = 2. | Click Save. | Saves. Required mode=Custom+Minimum persisted. In Answer: all marked rows must be filled AND at least 2 total rows must be filled. | NT | |
| FA-ED-020 | FAL required mode=None (all optional) — saves | Add FAL (5 rows). Set required mode = None. | Click Save. | Saves. Required mode=None persisted. In Answer, all rows are optional; respondent may skip all without error. | NT | |
| FA-ED-021 | FAL per-row type setting — set row 1 to Number type — saves | Add FAL (5 rows). Set per-row type for row 1 = Number. | Click Save. | Saves. Row 1 type=Number persisted; other rows remain default type. In Answer, row 1 accepts only numeric input. | NT | |
| FA-ED-022 | FAL per-row type mix — row 1 Number, row 2 Email, rows 3–5 String — saves | Add FAL (5 rows). Set row 1=Number, row 2=Email, rows 3–5=String. | Click Save. | Saves. Each row's type persisted independently. In Answer: row 1 numeric, row 2 email-format, rows 3–5 string input. | NT | |
...
| SAV-ED-013 | Survey question count limit — large matrix causes overflow count | Create survey. Add MTS with 30 SQs × 30 choices. Check editor's question count indicator. | Observe editor. | Editor displays a calculated "effective question count" exceeding the raw question count. If limit is exceeded, editor blocks save with error. *(Note: The claim that large matrix questions contribute additional count based on SQ/choice dimensions is unverified against spec — no explicit Confluence source found for the weighting formula. Treat as: "editor enforces a question count limit; large matrices may contribute more than 1 to the count.")* | NT | |
...
| PUB-ED-003 | Publish with validation errors — blocked | Survey with Q2 having empty question text. | Click Publish. | Editor blocks. *(Assumed to re-run all Save validations; additional publish-only checks may apply — confirm against spec.)* | NT | |
